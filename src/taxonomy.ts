...
function matchLen(haystack: string, keyword: string): number {
 ...
 */
function bestMatch<T extends { keywords: string[] }>(entries: T[], message: string): T | undefined {
...
export function matchGeography(message: string): Geography | undefined {
  return bestMatch(GEOGRAPHIES, message);
}

/** Best-matching sector in the message, else undefined. */
export function matchSector(message: string): Sector | undefined {
