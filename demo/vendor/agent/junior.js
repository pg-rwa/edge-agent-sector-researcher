 ...
 */
import { NOT_ADVICE } from "./master.js";
import { sourcesFor } from "./research/sources.js";
import { annualFundingPct, defaultMarketDeps, extractTickers, snapshotTickers, } from "./research/market-data.js";
import { fetchFilings, fetchMacro, fetchNews, } from "./research/feeds.js";
...
export class JuniorResearcher {
    ...
    feeds;
    idleIndex = 0;
    constructor(spec, market = defaultMarketDeps, 
    ...
    greeting() {
        ...
        return {
            text: `${s.codename} reporting for duty! ${this.face}\n\n` +
    ...
    sourceReport() {
        const lines = sourcesFor(this.spec, this.feeds).map((src) => {
            const state = src.availability === "live"
                ? "🟢 live"
     ...
     */
    async wireSection() {
        if (!this.feeds?.baseUrl)
            return "";
        const s = this.spec;
        const [news, filings, macro] = await Promise.all([
            fetchNews({ q: s.sector ?? s.geography, geoId: s.geographyId, sectorId: s.sectorId }, this.feeds).catch((e) => e),
            fetchFilings({ geoId: s.geographyId, sectorId: s.sectorId }, this.feeds).catch((e) => e),
            fetchMacro({ geoId: s.geographyId }, this.feeds).catch((e) => e),
        ]);
        const blocks = [];
        if (Array.isArray(news) && news.length > 0) {
            blocks.push("*Fresh off the wire:*\n" +
    ...
    async deepDive(message) {
        ...
        if (tickers.length === 0) {
            ...
            try {
                ...
                const rows = snaps.map((s) => {
                    const parts = [];
                    if (s.perp) {
                        parts.push(`oracle $${s.perp.oraclePx.toFixed(2)}`, `funding ${annualFundingPct(s.perp.fundingHourly).toFixed(1)}%/yr`, `max ${s.perp.maxLeverage}× perp`);
                    }
                    if (s.poolPriceUsd)
                        parts.push(`pool spot $${s.poolPriceUsd.toFixed(2)}`);
        ...
        const agendaSection = `*Research agenda* (from your brief):\n` +
        ...
        const focusNote = rest.filter((f) => f !== message).length && !this.feeds?.baseUrl
            ...
            : "";
        return {
            text: `📊 *Deep dive — ${this.spec.displayName}*\n\n` +
