 ...
 */
import { GEOGRAPHIES, SECTORS, matchGeography, matchSector, } from "./taxonomy.js";
export const NOT_ADVICE = "This is personal research you requested — not investment advice, and I'm read-only: I never touch wallets or funds.";
...
export class ResearchMaster {
    ...
    greeting() {
        return {
            text: "Hey, I'm the Research Master 🧭 — head of your personal research crew.\n\n" +
    ...
    handle(rawMessage) {
        ...
        if (CREW_CMD.test(message))
            return this.listCrew();
    ...
    handleCollect(message) {
        ...
        if (sector && !geo) {
            ...
            return {
                text: `${sector.label} — great beat. 🎯 Which geography should your junior cover?\n\n` +
                    `_${NOT_ADVICE}_`,
                suggestions: GEOGRAPHIES.map((g) => g.label),
            };
        }
        if (geo && !sector)
            return this.offerGeographyGeneralist(geo);
        // No recognizable beat at all.
        if (generalAsk)
            return this.refuseGeneralResearcher();
        return {
            text: "Hmm, I couldn't spot a geography or a sector in that. 🕵️ My juniors only work sharp beats — " +
    ...
    refuseGeneralResearcher() {
        return {
            text: "That one I have to refuse 🙅 — a *general researcher* that covers everything ends up knowing nothing, " +
     ...
     */
    offerGeographyGeneralist(geo) {
        this.phase = { kind: "confirm-geo-generalist", geo };
        return {
            text: `${geo.label} — solid hunting ground. 🗺️ Before I spin one up, a heads-up:\n\n` +
    ...
    handleGeoGeneralistConfirm(message, geo) {
        const sector = matchSector(message);
        if (sector)
            return this.beginIntake(geo, sector);
        if (YES.test(message)) {
            ...
            return {
                text: `📡 Junior *${junior.codename}* is live — your ${geo.label} generalist.\n\n` +
    ...
    handleNeedGeography(message, sector) {
        const geo = matchGeography(message);
        if (!geo) {
            return {
    ...
    beginIntake(geo, sector) {
        this.phase = { kind: "intake", geo, sector, step: "focus", answers: { focus: [] } };
        return {
            text: `${geo.label} × ${sector.label} — now *that's* a beat. 🎯\n\n` +
    ...
    handleIntake(message, phase) {
        ...
        this.phase = { kind: "collect" };
        return {
            text: `📡 Junior *${junior.codename}* is live — *${junior.displayName}*.\n\n` +
    ...
    spinUp(args) {
        ...
        const junior = {
            ...
            knowledgeNote: scope === "sector-specialist"
    ...
    listCrew() {
        ...
        const lines = this.juniors.map((j) => `• *${j.codename}* — ${j.displayName} (${j.scope === "sector-specialist" ? "specialist" : "generalist"})`);
        return {
            text: `Your research crew (${this.juniors.length}):\n${lines.join("\n")}\n\n` +
                `_${NOT_ADVICE}_`,
